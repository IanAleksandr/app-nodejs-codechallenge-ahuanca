...
export class SchemaRegistryService {
  ...
  private async loadSchema(topic: AvroTopic): Promise<string> {
  ...
  async getSchemaId(topic: AvroTopic): Promise<number> {
    ...
    const details = AVRO_SCHEMAS[topic];
    const schema = await this.loadSchema(topic);
    const { id } = await this.registry.register(
      { type: SchemaType.AVRO, schema },
      { subject: details.subject },
    );
    this.schemaCache.set(topic, { id, subject: details.subject });
    this.logger.debug(`Registered schema ${details.subject} with id ${id}`);
    return id;
  }
